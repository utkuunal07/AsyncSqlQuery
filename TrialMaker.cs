...
namespace SoftwareLocker
{
    ...
    public class TrialMaker
    {
        ...
        public RunTypes ShowDialog()
        {
            ...
            frmDialog PassDialog = new frmDialog(_BaseString, _Password, DaysToEnd(), _Runed, _Text, _Identifier);

            MakeHideFile();

            DialogResult DR = PassDialog.ShowDialog();
            if (PassDialog.MaxRaporCount!=null)
            {
                MaxRaporCount = PassDialog.MaxRaporCount[0].ToString() + PassDialog.MaxRaporCount[PassDialog.MaxRaporCount.Length - 1].ToString();
            }
        ...
        {
            _Password = _Password.Insert(0, MaxRaporCount[0].ToString());
            _Password = _Password.Insert(_Password.Length, MaxRaporCount[1].ToString());
        ...
        { 
            ...
            MaxRaporCount = Password[0].ToString() + Password[Password.Length - 1].ToString();
            ...
            string PasswordCarry = _Password;

            _Password = _Password.Remove(0, 1);
